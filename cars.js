...
const ferraris = (cars, brand) => {
    return cars.filter(car => car.brand === brand);
  };
  
const ferrariCars = ferraris(cars, 'Ferrari');
...
const ninetiesCars = nineties(cars)
console.log(ninetiesCars);
...
const bmwSum = () => {
  ...
  const prices = bmwCars.map(car => car.price);
  const sum = prices.reduce((total, price) => total + price, 0);
  return sum
}

const totalPriceBmw = bmwSum();
console.log(totalPriceBmw);
